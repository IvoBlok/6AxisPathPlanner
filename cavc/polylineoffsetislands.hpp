...
public:
  ...
  std::vector<cavc::Polyline2D<Real>> computeTwoSidedOffsets(std::vector<cavc::Polyline2D<Real>> innerBounds, std::vector<cavc::Polyline2D<Real>> outerBounds, Real offset);
  std::vector<cavc::Polyline2D<Real>> computeOneSidedOffsets(std::vector<cavc::Polyline2D<Real>> innerBounds, cavc::Polyline2D<Real> outerBound, Real offset);
...
template <typename Real>
std::vector<cavc::Polyline2D<Real>> ParallelOffsetIslands<Real>::computeTwoSidedOffsets(std::vector<cavc::Polyline2D<Real>> innerBounds, std::vector<cavc::Polyline2D<Real>> outerBounds, Real offset) {
  
  std::vector<cavc::Polyline2D<Real>> result;
  OffsetLoopSet<Real> loopSet;

  for (cavc::Polyline2D<Real>& innerBound : innerBounds) {
  ...
  for (cavc::Polyline2D<Real>& outerBound : outerBounds) {
