...
namespace MeshIntersect {
	...
	static double signedDistanceOfVertexToPlane(const core::Vector3<double>& vertex, const core::Plane<double>& plane) {
	...
	static const std::vector<double> signedDistancesToPlane(const std::vector<core::Vector3<double>>& vertices, const core::Plane<double>& plane) {
	...
	static bool getStartingItem(const std::vector<core::Polyline2D<double>>& items, std::vector<bool>& usedItems, core::Polyline2D<double>& startItem) {
	...
	static bool insertConnectingEdgePath(const std::vector<core::Polyline2D<double>>& paths, std::vector<bool>& usedPaths, core::Polyline2D<double>& currentChain) {
		int iPath = -1;
		for (auto& path : paths) {
			++iPath;
			if (usedPaths[iPath]) {
	...
	static void chainEdgePaths(std::vector<core::Polyline2D<double>>& paths) {
		if (paths.size() == 0) {
			return;
		}
	...
	static std::vector<core::Polyline2D<double>> constructGeometricPaths(const core::Plane<double>& plane, const std::vector<core::Vector3<double>>& vertices, const std::vector<core::Vector3<double>>& normals, const std::vector<EdgePath>& edgePaths, const std::vector<double>& vertexDistances) {
	...
	core::Polyline2D<double> getBasicBoxIntersection(core::Plane<double>& plane, StockInfo& stockInfo) {
		// create a mesh based on the info in 'stockInfo'

		core::ObjectShape stockShape;
		stockShape.vertices = std::vector<core::Vector3<double>>{
			stockInfo.zeroPoint,
			stockInfo.zeroPoint + 0.001f * stockInfo.width * core::Vector3<double>{1.f, 0.f, 0.f},
			stockInfo.zeroPoint + 0.001f * stockInfo.width * core::Vector3<double>{1.f, 0.f, 0.f} + 0.001f * stockInfo.height * core::Vector3<double>{0.f, 0.f, 1.f},
			stockInfo.zeroPoint + 0.001f * stockInfo.height * core::Vector3<double>{0.f, 0.f, 1.f},
			stockInfo.zeroPoint + 0.001f * stockInfo.length * core::Vector3<double>{0.f, 1.f, 0.f},
			stockInfo.zeroPoint + 0.001f * stockInfo.length * core::Vector3<double>{0.f, 1.f, 0.f} + 0.001f * stockInfo.width * core::Vector3<double>{1.f, 0.f, 0.f},
			stockInfo.zeroPoint + 0.001f * stockInfo.length * core::Vector3<double>{0.f, 1.f, 0.f} + 0.001f * stockInfo.width * core::Vector3<double>{1.f, 0.f, 0.f} + 0.001f * stockInfo.height * core::Vector3<double>{0.f, 0.f, 1.f},
			stockInfo.zeroPoint + 0.001f * stockInfo.length * core::Vector3<double>{0.f, 1.f, 0.f} + 0.001f * stockInfo.height * core::Vector3<double>{0.f, 0.f, 1.f}
		};

		stockShape.normals = std::vector<core::Vector3<double>>{
			core::Vector3<double>{0.f, 0.f, 0.f},
			core::Vector3<double>{0.f, 0.f, 0.f},
			core::Vector3<double>{0.f, 0.f, 0.f},
			core::Vector3<double>{0.f, 0.f, 0.f},
			core::Vector3<double>{0.f, 0.f, 0.f},
		...
		std::vector<core::Polyline2D<double>> result = getMeshPlaneIntersection(plane, stockShape);

		if (result.size() == 0) {
			return core::Polyline2D<double>{};
		}
