...
void RenderEngine::VulkanInternals::createPipelines() {
    ...
    createCurveOpaquePipeline();
    createCurveTransparentPipeline();
...
void RenderEngine::VulkanInternals::createCurveOpaquePipeline() {
    ...
    VkPushConstantRange psRange;
    psRange.offset = 0;
    psRange.size = sizeof(CurveShadersPushConstant);
    psRange.stageFlags = VK_SHADER_STAGE_VERTEX_BIT;
    ...
    if (vkCreatePipelineLayout(vulkanContext.device, &pipelineLayoutInfo, nullptr, &curveOpaquePipelineLayout) != VK_SUCCESS) {
        throw std::runtime_error("failed to create curve opaque pipeline layout!");
    }
    ...
    pipelineInfo.pColorBlendState = &colorBlending;
    pipelineInfo.pDynamicState = &dynamicState;
    pipelineInfo.layout = curveOpaquePipelineLayout;
    pipelineInfo.renderPass = renderPass;
    pipelineInfo.subpass = 0;
    ...
    // Actually create the VkPipeline
    if (vkCreateGraphicsPipelines(vulkanContext.device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &curveOpaquePipeline) != VK_SUCCESS) {
...
void RenderEngine::VulkanInternals::createCurveTransparentPipeline() {
    ...
    if (vkCreatePipelineLayout(vulkanContext.device, &pipelineLayoutInfo, nullptr, &curveTransparentPipelineLayout) != VK_SUCCESS) {
        throw std::runtime_error("failed to create curve transparent pipeline layout!");
    }
    ...
    pipelineInfo.pColorBlendState = &colorBlending;
    pipelineInfo.pDynamicState = &dynamicState;
    pipelineInfo.layout = curveTransparentPipelineLayout;
    pipelineInfo.renderPass = renderPass;
    pipelineInfo.subpass = 1;
    ...
    // Actually create the VkPipeline
    if (vkCreateGraphicsPipelines(vulkanContext.device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &curveTransparentPipeline) != VK_SUCCESS) {
        throw std::runtime_error("failed to create graphics pipeline!");
    }
...
void RenderEngine::VulkanInternals::createCompositePipeline() {
    ...
    if (vkCreatePipelineLayout(vulkanContext.device, &pipelineLayoutInfo, nullptr, &compositePipelineLayout) != VK_SUCCESS) {
...
void RenderEngine::VulkanInternals::recordCommandBuffer(std::list<std::shared_ptr<renderer::Curve>>& curves, std::list<std::shared_ptr<renderer::Object>>& objects, std::function<void()> drawGUIFunction, VkCommandBuffer commandBuffer, uint32_t imageIndex) {
    ...
    // ===============================================================
    // Subpass 0: opaque objects & curves
    // ===============================================================
    ...
    vkCmdSetViewport(commandBuffer, 0, 1, &viewport);
    vkCmdSetScissor(commandBuffer, 0, 1, &scissor);
    vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, curveOpaquePipelineLayout, 0, 1, &UBODescriptorSets[currentFrame], 0, nullptr);

    for (auto& curve : curves) {
        if (curve->isCurveRendered && curve->getTransparency() >= 0.99f)
    ...
    // curves
    vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, curveTransparentPipeline);
    vkCmdSetViewport(commandBuffer, 0, 1, &viewport);
    vkCmdSetScissor(commandBuffer, 0, 1, &scissor);
    vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, curveTransparentPipelineLayout, 0, 1, &UBODescriptorSets[currentFrame], 0, nullptr);
    ...
    vkCmdSetViewport(commandBuffer, 0, 1, &viewport);
    vkCmdSetScissor(commandBuffer, 0, 1, &scissor);
    vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, compositePipelineLayout, 0, 1, &compositeDescriptorSet, 0, nullptr);
...
void RenderEngine::VulkanInternals::cleanup(std::list<std::shared_ptr<renderer::Curve>>& curves, std::list<std::shared_ptr<renderer::Object>>& objects) {
    ...
    vkDestroyPipeline(vulkanContext.device, curveOpaquePipeline, nullptr);
    vkDestroyPipeline(vulkanContext.device, curveTransparentPipeline, nullptr);
    ...
    vkDestroyPipelineLayout(vulkanContext.device, curveOpaquePipelineLayout, nullptr);
    vkDestroyPipelineLayout(vulkanContext.device, curveTransparentPipelineLayout, nullptr);
    vkDestroyPipelineLayout(vulkanContext.device, compositePipelineLayout, nullptr);
